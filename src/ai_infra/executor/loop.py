...
class Executor:
    ...
    def __init__(
        ...
        *,
        ...
        self._mcp_manager: ExecutorMCPManager | None = None
    ...
    @property
    def mcp_manager(self) -> ExecutorMCPManager | None:
        """Get the MCP manager for external tool integration (Phase 15.3).
    ...
    async def _init_mcp(self) -> None:
        """Initialize MCP manager if servers configured (Phase 15.3).

        Creates the ExecutorMCPManager with configured servers and
        optionally runs auto-discovery based on config.mcp_auto_discover.
        """
        ...
        logger.info(f"Initializing MCP manager with {len(self._config.mcp_servers)} server(s)")

        self._mcp_manager = ExecutorMCPManager(
            configs=self._config.mcp_servers,
            discover_timeout=self._config.mcp_discover_timeout,
            tool_timeout=self._config.mcp_tool_timeout,
        )

        if self._config.mcp_auto_discover:
            try:
                ...
                # Don't fail executor startup - continue without MCP tools

        self._mcp_initialized = True

    def _get_mcp_tools(self) -> list[Any]:
        """Get discovered MCP tools (Phase 15.3).

        Returns:
            List of tools from MCP servers, or empty list if not available.
        """
        ...
        if tools:
            logger.debug(f"Including {len(tools)} MCP tools in agent context")
        return tools

    async def close_mcp(self) -> None:
        """Close MCP server connections (Phase 15.3).

        Should be called when the executor is done to clean up resources.
        """
    ...
    async def run(
        ...
        try:
            ...
            # Phase 15.3: Clean up MCP connections on error
