...
- **API Keys**: Never hardcode API keys. Use environment variables or secret managers.
- **Recursion Limits**: Always set `recursion_limit` on agents to prevent runaway costs.
- **Tool Results**: Truncate large tool results before sending to LLMs to prevent context overflow.
- **Input Validation**: Never pass untrusted input to `eval()` or similar functions.
- **MCP Servers**: Sanitize tool descriptions from external MCP servers to prevent prompt injection.
