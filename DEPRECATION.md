...
This document outlines how deprecated features are handled in `ai-infra`.
...
```python
import warnings
warnings.filterwarnings("default", category=DeprecationWarning, module="ai_infra")

# Now you'll see deprecation warnings when using deprecated features
from ai_infra.llm import OldClassName  # Warns: OldClassName is deprecated, use NewClassName
```
...
filterwarnings = [
    "error::DeprecationWarning:ai_infra.*",
]
```
...
```
DeprecationWarning: `CoreLLM.sync_chat()` is deprecated since v1.2.0 and will be
removed in v1.4.0. Use `CoreLLM.chat()` instead. See migration guide at:
https://docs.nfrax.com/ai-infra/migrations/1.2.0
```
...
- Open a [GitHub Discussion](https://github.com/nfraxlab/ai-infra/discussions)
